...
int main() {
     ...
     *****************************************************************************/
    char example[21] = "$DET,52343,16067*50\n";
    int b = 0;
    NegotiationData nTemp;
    nTemp.guess = 0;
    nTemp.encryptionKey = 0;
    GuessData gTemp;
    for (b = 0; b < 20; b++) {
        ProtocolDecode(example[b], &nTemp, &gTemp);
        printf("%c\n", example[b]);
    }
    printf("final step:%d,%d\n", nTemp.guess, nTemp.encryptionKey);
